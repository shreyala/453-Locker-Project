...
//  while (rfidWrite(4, (uint8_t*)"\x12\x34\x56\x78") != 0);  // Write specified bytes into address 3 (user EEPROM area)
//  Serial.println("Done!");
//  Serial.flush();
 
//  Serial.print("Enter Password");
//  while (rfidRead(rfidData, 3) != password); // Read the data  
//  Serial.print("Incorrect");
//  while (rfidRead(rfidData, 3) == password); // Read the data  
//  Serial.print("Correct");
//  for(idx=3;idx<=3;idx++)
//  {
  while (rfidRead(rfidData, 3) != 0); // Read the data  
    ...
    taginput = SaveHex(rfidData, 3);
    if(taginput == password)
    {
    Serial.print("Correct");
...
//    while(1);
//
//  for (idx = 3; idx <= 3; idx++) // For each valid address location
//  {
//    while (rfidRead(rfidData, idx) != 0); // Read the data  
//    //Serial.print(idx, DEC);
//    //Serial.print(": ");
//    //PrintHex(rfidData, 4);   // The rfidData string should now contain the data from the specific address, so display it on the Serial Monitor
//    password = SaveHex(rfidData, idx);
